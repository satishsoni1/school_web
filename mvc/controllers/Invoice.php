...
class Invoice extends Admin_Controller
{
    ...
    */
    ...
    // Add this function to your Invoice.php controller

    public function print_preview_multi()
    {
        if (permissionChecker('invoice_view')) {
            ...
            // Get IDs from URL (e.g., invoice/print_preview_multi/25-26)
            $idString = htmlentities(escapeString($this->uri->segment(3))); 
            $invoiceIDs = explode('-', $idString);

            $this->data['collection'] = []; // Array to hold multiple invoice data

            foreach ($invoiceIDs as $id) {
                if ((int)$id) {
                    $mainInvoiceData = []; // Temporary holder for this specific invoice

                    // Fetch Main Invoice
                    $mainInvoiceData['maininvoice'] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_maininvoiceID($id, $schoolyearID);

                    if (customCompute($mainInvoiceData['maininvoice'])) {
                        // Permission Check (Same as your original logic)
                        $canView = false;
                        if ($usertypeID == 1 || $usertypeID == 5) {
                            ...
                            if ($mainInvoiceData['maininvoice']->maininvoicestudentID == $getstudent->srstudentID) $canView = true;
                        } elseif ($usertypeID == 4) {
                            ...
                            if (in_array($mainInvoiceData['maininvoice']->maininvoicestudentID, $fetchStudent)) $canView = true;
                        }

                        if ($canView) {
                            // Fetch Sub Invoices
                            $mainInvoiceData['invoices'] = $this->invoice_m->get_order_by_invoice(['maininvoiceID' => $id]);
                            
                            // Fetch Payments & Totals
                            $mainInvoiceData['grandtotalandpayment'] = $this->grandtotalandpaidsingle($mainInvoiceData['maininvoice'], $schoolyearID, $mainInvoiceData['maininvoice']->maininvoicestudentID);
                            
                            // Fetch Student Info
                            $mainInvoiceData['student'] = $this->student_m->get_single_student(['studentID' => $mainInvoiceData['maininvoice']->maininvoicestudentID]);
                            
                            // Created By
                            $mainInvoiceData['createuser'] = getNameByUsertypeIDAndUserID($mainInvoiceData['maininvoice']->maininvoiceusertypeID, $mainInvoiceData['maininvoice']->maininvoiceuserID);

                            // Add to the main collection
                            $this->data['collection'][] = $mainInvoiceData;
                        }
                    }
                }
            }
            ...
            if (!empty($this->data['collection'])) {
                // Load the Multi-View
                $this->reportPDF('twinsprintmodule.css', $this->data, 'invoice/print_preview_multi');
            } else {
    ...
    public function print_previewviewpayment()
    {
        if(permissionChecker('invoice_view')) {
            ...
            $this->data["schoolyear"]   = $this->schoolyear_m->get_obj_schoolyear($schoolyearID);
            if((int)$globalpaymentID && (int)$maininvoiceID) {
                $globalpayment = $this->globalpayment_m->get_single_globalpayment_with_receipt_no([
                    'globalpaymentID' => $globalpaymentID,
                    'schoolyearID'    => $schoolyearID
                ]);
                $maininvoice   = $this->maininvoice_m->get_single_maininvoice([
                ...
                if(customCompute($maininvoice) && customCompute($globalpayment)) {
                    ...
                    if($f) {
                        ...
                        if(customCompute($studentrelation)) {
                            ...
                            $this->data['weaverandfines'] = pluck($this->weaverandfine_m->get_order_by_weaverandfine(['globalpaymentID' => $globalpaymentID]), 'obj', 'paymentID');
                          //  
                            $this->data['discount_calculate'] = 0;
                            $this->data['paymenttype'] = '';
                            if(customCompute($this->data['payments'])) {
                                foreach($this->data['payments'] as $payment) {
                                    $this->data['paymenttype'] = $payment->paymenttype;
                            ...
                            if(customCompute($this->data['invoices'])) {
                                foreach($this->data['invoices'] as $invoice) {
                                    if($invoice->discount > 0) { 
                                        $this->data['discount_calculate'] += (($invoice->amount/100) * $invoice->discount);
                                    }
                                }
                            }
                            ...
                            $this->data['globalpayment']   = $globalpayment;
                            $this->data['maininvoice']     = $maininvoice;
                            $this->data['all_maininvoices']         = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_studentID($globalpayment->studentID, $schoolyearID);
                            $this->data['grandtotalandpayment'] = $this->grandtotalandpaid($this->data['all_maininvoices'], $schoolyearID);
                            
                            $this->data['classes'] = pluck($this->classes_m->get_classes(), 'classes_without_div', 'classesID');
                           // var_dump($this->data['weaverandfines']);die;
                            $this->reportPDF('invoicemodule.css', $this->data, 'invoice/print_previewviewpayment');
                        } else {
                            $this->data["subview"] = "error";
                            $this->load->view('_layout_main', $this->data);
                        }
                ...
                if(customCompute($paymentlists)) {
                    ...
                    foreach($paymentlists as $key => $paymentlist) {
                        ...
                        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
                            if(('CASH' == $paymentlist['paymenttype']) || ('Online' == $paymentlist['paymenttype']) || ($this->lang->line('Online') == $paymentlist['paymenttype']) || ($this->lang->line('Cash') == $paymentlist['paymenttype']) || ($this->lang->line('Cheque') == $paymentlist['paymenttype']) || ('Cash' == $paymentlist['paymenttype']) || ('Cheque' == $paymentlist['paymenttype'])) {
            ...
            if(permissionChecker('invoice_delete')) {
                if((int)$globalpaymentID && (int)$maininvoiceID) {
                    ...
                    if(customCompute($globalpayment)) {
                        ...
                        $excType = TRUE;
                        foreach($payments as $payment) {
                            if(('CASH' == $payment->paymenttype) || ($this->lang->line('Online') == $payment->paymenttype) || ('Online' == $payment->paymenttype) || ($this->lang->line('Cash') == $payment->paymenttype) || ($this->lang->line('Cheque') == $payment->paymenttype) || ('Cash' == $payment->paymenttype) || ('Cheque' == $payment->paymenttype)) {
