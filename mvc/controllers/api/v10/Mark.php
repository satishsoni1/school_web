    ...
    private function getMark($studentID, $classesID) {
        if((int)$studentID && (int)$classesID) {
            ...
            if(customCompute($student) && customCompute($classes)) {
                ...
                $markpercentagestotest   = pluck($markpercentages, 'obj', 'markpercentageID');
                foreach ($allStudentMarks as $value) {
                    $highestMarks[$value->examID][$value->subjectID][$value->markpercentageID] = $markpercentagestotest[$value->markpercentageID]->percentage;
