...
class Backendmenucall extends Api_Controller
{
    ...
    private function menuTree($dataset, $sessionPermission) {
        ...
        foreach ($dataset as $id => &$node) {
            if(!in_array($node['menuName'], $hideMenu) && ($node['link'] == '#' || (isset($sessionPermission[$node['link']]) && $sessionPermission[$node['link']] != "no"))) {
                if ($node['parentID'] == 0) {
                    $tree[$id]=&$node;
                } else {
        ...
        $treeGenerate = [];
        if(customCompute($newTree)) {
            $i = 1;
            foreach ($newTree as $t) {
                if (isset($t['child']) && customCompute($t['child']) > 1) {
                ...
                } elseif (isset($t['child']) && customCompute($t['child']) == 1) {
                    foreach ($t['child'] as $c) {
