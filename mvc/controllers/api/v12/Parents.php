...
class Parents extends Api_Controller 
{
    ...
    {
        $myProfile = false;
        if($this->session->userdata('usertypeID') == 4 && !permissionChecker('parents_view')) {
            $myProfile = true;
        }
        ...
        if($this->session->userdata('usertypeID') == 4 && $myProfile) {
            ...
            $this->retdata['parents'] = customCompute($parents) ? $parents : [];
        }
    ...
    {
        $this->retdata['profile'] = customCompute($parents) ? $parents : [];
    }
