...
*/
	...
	public function index() {
		...
		if($usertypeID == 3) {
			...
			if((int)$id) {
				$this->data['set'] = $id;
				$this->data['payments'] = $this->payment_m->get_payment_with_studentrelation_1($schoolyearID,$id);
			}else{
	...
	public function payment_list() {
		...
		if((int)$classID) {
			$string = base_url("paymenthistory/index/$classID");
			echo $string;
		} else {
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('defaultschoolyearID') == 5)) {
			...
			if((int)$id) {
				...
				$this->data['globalpayment'] = $this->globalpayment_m->get_single_globalpayment(array('globalpaymentID' => $this->data['payment']->globalpaymentID));
								...
								if ($this->form_validation->run() == FALSE) {
									...
									if(customCompute($maininvoice)) {
										...
										$globalpaymentArray = array(
											'auto_receipt_no' => $this->input->post('auto_receipt_no'),
