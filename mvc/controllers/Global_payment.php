...
class Global_payment extends Admin_Controller
{
    ...
    */
    ...
    public function incrementCode($code) {
        ...
        if (!empty($matches[1])) {
            $num_length = strlen($matches[1]); // Preserve original length
            $new_number = str_pad($matches[1] + 1, $num_length, '0', STR_PAD_LEFT);
            
            return preg_replace('/\d+$/', $new_number, $code);
        }
    ...
    public function index() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->data['single_student_info'] = $this->student_m->get_student(array('studentID' => $studentID, 'srschoolyearID' => $schoolyearID));
                ...
                if(customCompute($this->data['single_student'])) {
                   ...
                   // var_dump($this->data['invoices']);die;
            ...
            foreach ($payments as $payment) {
                ...
                if(isset($returnArray['paid'][$payment->globalpaymentID])) {
                    ...
                    if(!isset($returnArray['paiddate'][$payment->globalpaymentID])) {
                        $returnArray['paiddate'][$payment->globalpaymentID] = ($payment->payment_m_date=="" || $payment->payment_m_date==null)?$payment->paymentdate: $payment->payment_m_date;
                    }
                }
            }
        }
    ...
    public function paymentSend() {
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $globalpayment_max = $this->globalpayment_m->get_count_globalpayment($schoolyearID);
                ...
                if($schoolyearID != 2){
                    if(isset($globalpayment_max->auto_receipt_no) && $globalpayment_max->auto_receipt_no != ""){
                        $globalpayment['auto_receipt_no']      = $this->incrementCode($globalpayment_max->auto_receipt_no);
                    }else{
                        if($schoolyearID == 4){
                            $globalpayment['auto_receipt_no']      = "2026-27/0001";
                        }else{
                            $globalpayment['auto_receipt_no']      = "2025-26/0001";
                        }
                        
                    }
                }else{
                    $globalpayment['auto_receipt_no']      = "2024-25/".$globalpayment_max->globalpaymentID+1;
                }
                $globalpayment['payment_date'] = $payment_date; 
                if($studentID) {
