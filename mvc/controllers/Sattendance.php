...
*/
			...
			if((int)$id) {
				...
				if(isset($fetchClass[$id])) {
					if(customCompute($this->data['students'])) {
						$this->data['allsection'] = $this->studentrelation_m->get_order_by_student(array('srclassesID' => $id, 'srschoolyearID' => $schoolyearID));
					} else {
						$this->data['students'] = [];
					}
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if(customCompute($students)) {
						if($this->data['setting']->attendance == "subject") {
							$attendance_monthyear = pluck($this->subjectattendance_m->get_order_by_sub_attendance(array('schoolyearID' => $schoolyearID, "classesID" => $classesID, "subjectID" => $subjectID, "monthyear" => $monthyear)), 'obj', 'studentID');
						} else {
							$attendance_monthyear = pluck($this->sattendance_m->get_order_by_attendance(array('schoolyearID' => $schoolyearID, "classesID" => $classesID,"monthyear" => $monthyear)), 'obj', 'studentID');
						}
						...
						if($this->data['setting']->attendance == "subject") {
							$this->data['attendances'] = pluck($this->subjectattendance_m->get_order_by_sub_attendance(array('classesID' => $classesID, 'subjectID' => $subjectID, 'schoolyearID' => $schoolyearID, 'monthyear' => $monthyear)), 'obj', 'studentID');
						} else {
							$this->data['attendances'] = pluck($this->sattendance_m->get_order_by_attendance(array('classesID' => $classesID, 'schoolyearID' => $schoolyearID, 'monthyear' => $monthyear)), 'obj', 'studentID');
						}
					}
	...
	public function save_attendace() {
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($this->data['setting']->attendance == "subject") {
					$data = array('a'.abs($day) => "A",'schoolyearID' => $schoolyearID,'classesID'=>$classes,'monthyear'=>$monthyear,'subjectID'=>$subjectID);
				...
				if($f && customCompute($students)) {
					if($messageType == 'email') {
						$this->sendAbsentEmail($students, $schoolyearID, $classes);
					} elseif($messageType == 'sms') {
						$this->sendAbsentSMS($students, $schoolyearID, $classes);
					}
				}
	...
	private function sendAbsentEmail($students, $schoolyearID, $classesID) {
		...
		$objStudents = pluck($this->studentrelation_m->get_order_by_student(array('srschoolyearID'=> $schoolyearID, 'srclassesID'=> $classesID), TRUE),'obj','srstudentID');
	...
	private function sendAbsentSMS($students, $schoolyearID, $classesID) {
	...
	public function delete_day_attendance() {
		$retArray['status'] = FALSE;
		$retArray['message'] = '';
			...
			if((int)$classesID && $date != "") {
				$explode_date = explode("-", $date);
				$day = (int)$explode_date[0];
				$monthyear = $explode_date[1]."-".$explode_date[2];
				$column = 'a'.$day;
	
				// Prepare the data to set the specific day to NULL
				$updateData = array($column => NULL);
				
				// Perform update based on attendance setting (Subject vs Daily)
				if($this->data['setting']->attendance == "subject") {
					$subjectID = $this->input->post('subjectID');
					$this->db->where([
						'classesID'    => $classesID,
					...
					$this->db->update('subjectattendance', $updateData);
				} else {
					$this->db->where([
						'classesID'    => $classesID,
						'monthyear'    => $monthyear,
				...
				$retArray['status'] = TRUE;
				$retArray['message'] = "Attendance for $date has been cleared.";
			} else {
				$retArray['message'] = "Invalid Parameters Provided.";
			}
