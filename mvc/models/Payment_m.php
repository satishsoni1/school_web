...
class Payment_m extends MY_Model {
	...
	public function get_payment_with_studentrelation($schoolyearID) {
		$this->db->select('payment.*,if(globalpayment.payment_receipt_no is null,globalpayment.auto_receipt_no,globalpayment.payment_receipt_no) as payment_receipt_no,globalpayment.payment_date, invoice.invoiceID, invoice.feetype, invoice.amount, studentrelation.*');
		$this->db->from('payment');
		$this->db->join('globalpayment', 'globalpayment.globalpaymentID = payment.globalpaymentID');
		$this->db->join('studentrelation', 'studentrelation.srstudentID = payment.studentID AND studentrelation.srschoolyearID = payment.schoolyearID', 'LEFT');
		$this->db->join('invoice', 'invoice.invoiceID = payment.invoiceID', 'LEFT');
		$this->db->where(array('payment.schoolyearID' => $schoolyearID));
		$this->db->order_by($this->_order_by);
	...
	public function get_payment_with_studentrelation_1($schoolyearID,$id) {
		$this->db->select('payment.*,if(globalpayment.payment_receipt_no is null,globalpayment.auto_receipt_no,globalpayment.payment_receipt_no) as payment_receipt_no,globalpayment.payment_date,invoice.invoiceID, invoice.feetype, invoice.amount, studentrelation.*');
		$this->db->from('payment');
		$this->db->join('globalpayment', 'globalpayment.globalpaymentID = payment.globalpaymentID');
		$this->db->join('studentrelation', 'studentrelation.srstudentID = payment.studentID AND studentrelation.srschoolyearID = payment.schoolyearID', 'LEFT');
		$this->db->join('invoice', 'invoice.invoiceID = payment.invoiceID', 'LEFT');
		$this->db->where(array('payment.schoolyearID' => $schoolyearID));
		$this->db->where(array('studentrelation.srclassesID' => $id));
		$this->db->order_by($this->_order_by);
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function get_payments($array) {
		...
		$this->db->join('globalpayment','globalpayment.globalpaymentID = payment.globalpaymentID');
		$this->db->where('globalpayment.payment_date >=',$array['fromdate']);
		$this->db->where('globalpayment.payment_date <=',$array['todate']);
