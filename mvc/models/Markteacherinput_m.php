...
class Markteacherinput_m extends MY_Model {

	protected $_table_name = 'mark_teacher_input';
	...
	public function student_all_optional_mark_array($array) {
		$this->db->select('*');
		$this->db->from('mark_optional');
		
		if(isset($array['subjectID'])) {
			$this->db->where('mark_optional.subjectID', $array['subjectID']);
		}

		if(isset($array['schoolyearID'])) {
			$this->db->where('mark_optional.schoolyearID', $array['schoolyearID']);
		}

		if(isset($array['examID'])) {
			$this->db->where('mark_optional.examID', $array['examID']);
		}

		if(isset($array['classesID'])) {
			$this->db->where('mark_optional.classesID', $array['classesID']);
		}

		if(isset($array['studentID'])) {
			$this->db->where('mark_optional.studentID', $array['studentID']);
		}
