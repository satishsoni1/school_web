...
class Studentrelation_m extends MY_Model {
	...
	private function userRelation() {
		...
		if($usertypeID == 2) {
			...
			$this->db->from('subjectteacher')->where(array('teacherID' => $userID))->order_by('classesID');
			...
			return mergeArray(mergeArray($classResult, $routineResult, 'classesID'),$classResult1, 'classesID');
		} elseif($usertypeID == 3) {
	...
	public function general_get_single_student($arrays = [], $studentExtend = FALSE) {
        ...
        $this->db->select("studentrelation.*,student.*,CONCAT(student.name, ' ', student.surname) as fullname");
        $this->db->from('studentrelation');
        $this->db->join('student', 'student.studentID = studentrelation.srstudentID', 'LEFT');
	...
	public function getsectionID($classesID) {
	...
	public function get_order_by_studentrelation($array=NULL) {
		$this->db->order_by('srclasses','ASC');
