<?php
    ...
    function calculate_attendace($student_id,$schoolyearID) {
        $CI = &get_instance();
        $CI->load->model('Sattendance_m');
        $attendances =  pluck($CI->Sattendance_m->get_order_by_attendance((array('studentID' => $student_id, 'schoolyearID' => $schoolyearID))),'obj','studentID');
        
        return $attendances;
    }
    function calculate_attendace_total($student_id, $schoolyearID)
{
    $CI = &get_instance();
    $CI->load->model('Sattendance_m');
    ...
    function markCalculationView( $getmark, $subjectFinalmark, $percentageMark = 100 )
    {
        if ( $getmark == 0 || $getmark == '' ) {
            ...
            if ( $retMark == 0 ) {
                ...
                //return floor($retMark,2);
                return round($retMark,2);
            }
        }
    }
